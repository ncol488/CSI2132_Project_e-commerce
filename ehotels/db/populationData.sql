...
-- make employeeid auto-generate in the DB and sync the sequence to the current max employee id

CREATE SEQUENCE IF NOT EXISTS ehotels.employee_employeeid_seq;

ALTER TABLE ehotels.employee
ALTER COLUMN employeeid
SET DEFAULT nextval('ehotels.employee_employeeid_seq');

SELECT setval(
  'ehotels.employee_employeeid_seq',
  COALESCE((SELECT MAX(employeeid) FROM ehotels.employee), 1)
);
...
-- make auto-generated default for customerid and sync the sequence to your current max customer id

CREATE SEQUENCE IF NOT EXISTS ehotels.customer_customerid_seq;

ALTER TABLE ehotels.customer
ALTER COLUMN customerid
SET DEFAULT nextval('ehotels.customer_customerid_seq');

SELECT setval(
  'ehotels.customer_customerid_seq',
  COALESCE((SELECT MAX(customerid) FROM ehotels.customer), 1)
);
